 ...
 */
typedef union dfi_pipe_cfg_t
{
    struct
    {
        uint32_t pipe_en: 4;
        uint32_t post_gb_fc_dly: 4;
        uint32_t gb_mode: 4;
    };
    uint32_t val;
} dfi_pipe_cfg_t;
 ...
 *          end of clock pulse to extend width by configured number of phases.
 *
 * @note    This structure directly maps to the CSR layout. Please do not
 *          modify unless the CSR fields change.
 *
 ...
 *
 * @details DFI Override Traffic configuration. Configures dynamic clock
 ...
 *
 * paden_pext   Pad enable Pulse Extension configuration.
 * clken_pext   Clock Enale Pulse Extension configuration.
 * over_traffic Traffic override configuration.
 * ckctrl       Clock Control pipe stage configuration.
 * wckctrl      WCK control pipe stage configuration.
 * wctrl        Write control pipe stage configuration.
 * wenctrl      Write enable control pipe stage confgiuration.
 * wrctrl       Write clock control pipe stage configuration.
 * wrc          Write clock pipe stage configuration.
 * wrd          Write data pipe stage configuration.
 * rcctrl       Read control pipe stage configuration.
 * rdd          Read data pipe stage configuration.
 */
